...
| :--- | :--- | :--- |
| **Alina** | CI/CD & Data | <ul><li>Finalized, tested, and debugged the `ci.yml` workflow (D4).</li><li>Trained the final production model (`best.pt`)</li> <li>Set up DVC for data versioning.</li><li>Created the initial repository and project structure.</li><li>Modified and debugged `app.py`.</li><li>Ran initial YOLO model tests.</li></ul> |
...
| **Rafsha**| API, Infra, & Testing | <ul><li>Wrote the `Dockerfile` and `docker-compose.yml` (D3).</li><li>Wrote the `test_api.py` unit tests (D4b).</li><li>Set up `pre-commit` hooks (D6).</li><li>Modified `app.py` to comply with docker and made the calorie lookup function for prediction .</li><li>Created the initial `ci.yml` file.</li><li>Wrote the `README.md` (D1) and `CONTRIBUTING.md` and `codeofconduct.md` file (D2).</li></ul> |
