...
| **Rafsha**| API, Infra, & Testing | <ul><li>Wrote the `Dockerfile` and `docker-compose.yml` (D3).</li><li>Wrote the `test_api.py` unit tests (D4b).</li><li>Set up `pre-commit` hooks (D6).</li><li>Modified `app.py` to comply with docker and made the calorie lookup function for prediction .</li><li>Created the initial `ci.yml` file and did some debugging commits.</li><li>Wrote the `README.md` (D1) and `CONTRIBUTING.md` (D2).<li>corrected and updated repo structure.</li><li>corrected and preprocessed dataset to comply with YOLO.</li></ul> |
