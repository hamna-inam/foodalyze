...
BASE_PATH = Path('/Users/bstar/Documents/Fall25/MLOps/MLFlow/IndianFoodDatasetFinalFiltered')
# Ensure the root path exists
if not os.path.exists(BASE_PATH):
    print(f"FATAL ERROR: Data directory not found at {BASE_PATH}")
    print("Please adjust the BASE_PATH variable to point to your 'IndianFoodDatasetFinalFiltered' folder.")
    exit()
...
def convert_labels_to_yolo_ids():
    ...
    for split in ["test"]:
        ...
        if not labels_dir.exists():
            print(f"⚠️ Warning: Labels directory not found for {split} at {labels_dir}. Skipping.")
            continue
        ...
        print(f"✅ Conversion complete for {split} split. Processed {count} label files.")
